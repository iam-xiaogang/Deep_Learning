...
def tarin_verify_data_process():
    ...
    train_loder = DataLoader(train_data,batch_size=64,shuffle=True,num_workers=4)
...
def matplotlib_imshow(data):
    ...
    plt.subplot(1,2,1)
    plt.plot(data['epoch'],data.train_loss_all,'r-',label='train loss')
    plt.plot(data['epoch'],data.val_loss_all,'b-',label='val loss')
    plt.legend()
    plt.xlabel('epoch')
    plt.ylabel('loss')

    plt.subplot(1, 2, 2)
    plt.plot(data['epoch'], data.train_acc_all, 'r-', label='train acc')
    plt.plot(data['epoch'], data.val_acc_all, 'b-', label='val acc')
    plt.legend()
    plt.xlabel('epoch')
    plt.ylabel('accuracy')
    plt.show()
